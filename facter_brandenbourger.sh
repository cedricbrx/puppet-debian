...
GATEWAY_MAC_ADDRESS=`ip n show | grep $(ip route show default | awk '/default/ {print $3}') | awk '{print $5}'`
...
if [ -n "$WIRELESS_NETWORK_VENDOR" -a  "$WIRELESS_NETWORK_VENDOR" == "intel" ]; then
